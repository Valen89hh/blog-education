...
import SelectImage, { ImageSelectData } from "./select-image";
...
const validators: Validators<PostShema> = {
    ...
    image: (value)=>{
        if(!value.file && !value.imgUrl) return "La imagen es requerida"
        else return null
    },
...
const EditorPost: React.FC<EditorPostProps> = ({
    ...
    const {handleValidateSubmit, setFieldValue, fields} = useFormFields<PostShema>(initialValues,validators)
            ...
            handleValidateSubmit((data)=>{
                ...
                startLoadingSave(async()=>{
                    ...
                    await updatePost(formData, {
                        ...
                        status: status,
                        post_id: idPost,
                        image_path: initialValues.image.imgPath!,
                        image_url: initialValues.image.imgUrl!
                    })
    ...
    return ( 
        <div className="flex flex-col min-h-screen bg-[#FAFAFA]">
            <header ref={headerRef} className="px-4 py-2 flex justify-between">
                ...
                <h2 className="font-medium text-lg">{idPost ? "Editar" : "Crear"} Post</h2>
            ...
            <Container className="flex-1 flex flex-col">
                    ...
                    >
                        ...
                        <div className="px-8 py-6 space-y-6">
                            <SelectImage 
                                onChangeImage={(imgSelect)=>setFieldValue("image", imgSelect)}
                            ...
                            />
                            <SelectCategory 
                                onChangeCategory={cat=>setFieldValue("category", cat)}
                                value={fields.category.value}
                            />
                        </div>
                        <RichText content={initialValues.content} onChange={value=>setFieldValue("content", value)}/>
                    </Card>
